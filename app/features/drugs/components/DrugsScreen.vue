  ...
  <div class="flex flex-col gap-8">
    ...
    />
     ...
     <n-modal
      ...
      class="max-w-md rounded-3xl overflow-hidden shadow-2xl"
    >
      <div v-if="viewingDrug" class="flex flex-col gap-4 py-2">
        <!-- Başlıq -->
        <div class="flex items-center justify-between">
          ...
          <n-tag type="info" size="small" round class="font-mono"
            >#{{ viewingDrug.id }}</n-tag
          >
        </div>

        <!-- Body -->
        <div
          v-if="viewingDrug.body"
        ...
        >
          <div class="flex justify-between items-center text-xs">
            ...
            <span class="text-slate-700 font-semibold">
              {{
                new Date(viewingDrug.created_at).toLocaleDateString("az-AZ")
              }}
            </span>
          </div>
          <div class="flex justify-between items-center text-xs">
            ...
            <span class="text-slate-700 font-semibold">
              {{
                new Date(viewingDrug.updated_at).toLocaleDateString("az-AZ")
              }}
            </span>
          </div>
        </div>
      ...
      <template #action>
        <div class="flex justify-end gap-3">
          ...
          <n-button
            type="primary"
            @click="() => { showViewModal = false; openEditModal(viewingDrug!) }"
          >
            Redaktə et
          </n-button>
        </div>
...
const viewingDrug = ref<Drug | null>(null);

const openViewModal = (row: Drug) => {
  viewingDrug.value = row;
...
const handleSubmit = async () => {
  ...
  // clearNuxtData yoxdur — sadəcə refresh
  refresh();
};
      ...
      h("div", { class: "flex items-center gap-3" }, [
        h(
          NAvatar,
          {
            ...
            size: 32,
            color: "#F0F9FF",
            style: "color:#0284C7;font-weight:800;border:1px solid #E0F2FE",
          },
          { default: () => row.title[0] }
        ),
