<template>
  <div class="flex flex-col gap-8">
    ...
    <div class="flex items-end justify-between border-b border-slate-100 pb-6">
      <div class="space-y-1">
        <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight">{{ t('symptoms.title') }}</h2>
        <p class="text-slate-500 text-sm font-medium">Manage symptoms in the healthcare network.</p>
      </div>
...
>
      ...
      <n-spin :show="isLoadingLang && activeTab === 'az'">
        <div class="flex flex-col gap-4 pt-4">
          <n-form-item label="Ad (AZ)">
            <n-input v-model:value="modalForm.title.az" placeholder="Simptom adı..." size="large" class="rounded-xl" />
          </n-form-item>
        </div>
      </n-spin>
    </n-tab-pane>
      ...
      <n-spin :show="isLoadingLang && activeTab === 'en'">
        <div class="flex flex-col gap-4 pt-4">
          <n-form-item label="Name (EN)">
            <n-input v-model:value="modalForm.title.en" placeholder="Symptom name..." size="large" class="rounded-xl" />
          </n-form-item>
        </div>
      </n-spin>
    </n-tab-pane>
      ...
      <n-spin :show="isLoadingLang && activeTab === 'ru'">
        <div class="flex flex-col gap-4 pt-4">
          <n-form-item label="Название (RU)">
            <n-input v-model:value="modalForm.title.ru" placeholder="Название симптома..." size="large" class="rounded-xl" />
          </n-form-item>
        </div>
      </n-spin>
    </n-tab-pane>
  ...
  <div class="grid grid-cols-2 gap-4 mt-4">
    <n-form-item label="Gender *">
      <n-select v-model:value="modalForm.gender" :options="genderOptions" size="large" />
    </n-form-item>

    <n-form-item label="Side *">
      <n-select v-model:value="modalForm.side" :options="sideOptions" size="large" />
    </n-form-item>
  </div>

  <n-form-item label="Body Part *">
    <n-input v-model:value="modalForm.body_part" placeholder="məs. head, chest..." size="large" class="rounded-xl" />
  </n-form-item>
...
import { useSymptoms, useCreateSymptom, useUpdateSymptom, useDeleteSymptom } from '../composables/useSymptoms'
...
const columns: DataTableColumns<Symptom> = [
  ...
  },
  {
    ...
    render: (row) =>
      h('div', { class: 'flex items-center gap-3' }, [
        h(NAvatar, { round: true, size: 32, color: '#FEFCE8', style: 'color:#CA8A04;font-weight:800;border:1px solid #FEF9C3' },
          { default: () => row.title[0] }),
  ...
  },
  {
    ...
    render: (row) =>
      h(NTag, {
        type: row.gender === 'male' ? 'info' : row.gender === 'female' ? 'warning' : 'default',
  ...
  },
  {
    title: 'Side',
    key: 'side',
    render: (row) => h('span', { class: 'text-slate-600 text-sm capitalize' }, row.side),
  },
  {
    title: 'Body Part',
    key: 'body_part',
    render: (row) => h('span', { class: 'text-slate-600 text-sm' }, row.body_part),
  },
  {
    title: 'Actions',
    key: 'actions',
