<template>
  <div class="flex flex-col gap-8">
    ...
    <div class="flex items-end justify-between border-b border-slate-100 pb-6">
      <div class="space-y-1">
        <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight">Clinics</h2>
        <p class="text-slate-500 text-sm font-medium">Manage clinics across all regions in the healthcare network.</p>
      </div>
          ...
          <n-spin :show="isLoadingLang && activeTab === 'az'">
            <div class="flex flex-col gap-4 pt-4">
              <n-form-item label="Ad (AZ)">
                <n-input v-model:value="modalForm.title.az" placeholder="Klinika adı..." size="large" class="rounded-xl" />
              </n-form-item>
              <n-form-item label="Təsvir (AZ)">
                <n-input v-model:value="modalForm.description.az" type="textarea" placeholder="Haqqında..." :rows="2" />
              </n-form-item>
              <n-form-item label="Ünvan (AZ)">
                <n-input v-model:value="modalForm.address.az" placeholder="Ünvan..." size="large" class="rounded-xl" />
              </n-form-item>
            </div>
          </n-spin>
        </n-tab-pane>
          ...
          <n-spin :show="isLoadingLang && activeTab === 'en'">
            <div class="flex flex-col gap-4 pt-4">
              <n-form-item label="Name (EN)">
                <n-input v-model:value="modalForm.title.en" placeholder="Clinic name..." size="large" class="rounded-xl" />
              </n-form-item>
              <n-form-item label="Description (EN)">
                <n-input v-model:value="modalForm.description.en" type="textarea" placeholder="Description..." :rows="2" />
              </n-form-item>
              <n-form-item label="Address (EN)">
                <n-input v-model:value="modalForm.address.en" placeholder="Address..." size="large" class="rounded-xl" />
              </n-form-item>
            </div>
          </n-spin>
        </n-tab-pane>
          ...
          <n-spin :show="isLoadingLang && activeTab === 'ru'">
            <div class="flex flex-col gap-4 pt-4">
              <n-form-item label="Название (RU)">
                <n-input v-model:value="modalForm.title.ru" placeholder="Название клиники..." size="large" class="rounded-xl" />
              </n-form-item>
              <n-form-item label="Описание (RU)">
                <n-input v-model:value="modalForm.description.ru" type="textarea" placeholder="Описание..." :rows="2" />
              </n-form-item>
              <n-form-item label="Адрес (RU)">
                <n-input v-model:value="modalForm.address.ru" placeholder="Адрес..." size="large" class="rounded-xl" />
              </n-form-item>
            </div>
          </n-spin>
        </n-tab-pane>
...
/>
        ...
        <div class="grid grid-cols-2 gap-4">
          <n-form-item label="Latitude">
            <n-input v-model:value="modalForm.latitude" placeholder="40.4093" size="large" class="rounded-xl" />
          </n-form-item>
          <n-form-item label="Longitude">
            <n-input v-model:value="modalForm.longitude" placeholder="49.8671" size="large" class="rounded-xl" />
          </n-form-item>
        </div>
        <div class="flex items-center gap-2">
...
const fetchLangData = async (id: number, lang: string) => {
  ...
  try {
    ...
    const data = await $fetch<{ data: Clinic }>(`https://icheckapi.200soft.com/api/v1/clinics/${id}/`, {
      headers: {
    ...
    modalForm.description[lang as 'az' | 'en' | 'ru'] = data.data.description ?? ''
...
const columns: DataTableColumns<Clinic> = [
  ...
  },
  {
    ...
    render: (row) =>
      h('div', { class: 'flex items-center gap-3' }, [
        h(NAvatar, { round: true, size: 32, color: '#F0FDF4', style: 'color:#16A34A;font-weight:800;border:1px solid #DCFCE7' },
          { default: () => row.title[0] }),
  ...
  },
  {
    ...
    render: (row) => {
      ...
      return h('span', { class: 'text-slate-600 text-sm' }, region?.title ?? '—')
    },
  },
  {
    title: 'Phone',
    key: 'phone',
    render: (row) => h('span', { class: 'text-slate-600 text-sm' }, row.phone ?? '—'),
  },
  {
    title: 'Top',
    key: 'top',
