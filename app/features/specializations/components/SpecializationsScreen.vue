<template>
  <div class="flex flex-col gap-8">
    ...
    <div class="flex items-end justify-between border-b border-slate-100 pb-6">
      <div class="space-y-1">
        <h2 class="text-3xl font-extrabold text-slate-900 tracking-tight">Specializations</h2>
        <p class="text-slate-500 text-sm font-medium">Manage medical specializations in the healthcare network.</p>
      </div>
    ...
    <n-card :bordered="false" class="border-none shadow-sm rounded-2xl overflow-hidden bg-white">
      <div class="mb-6 flex items-center justify-between gap-4 p-1">
        <n-input v-model:value="searchQuery" placeholder="Search specializations..." size="large" class="rounded-xl max-w-sm" clearable>
          ...
          <n-spin :show="isLoadingLang && activeTab === 'az'">
            <div class="flex flex-col gap-4 pt-4">
              <n-form-item label="Ad (AZ)">
                <n-input v-model:value="modalForm.title.az" placeholder="İxtisas adı..." size="large" class="rounded-xl" />
              </n-form-item>
            </div>
          </n-spin>
        </n-tab-pane>
          ...
          <n-spin :show="isLoadingLang && activeTab === 'en'">
            <div class="flex flex-col gap-4 pt-4">
              <n-form-item label="Name (EN)">
                <n-input v-model:value="modalForm.title.en" placeholder="Specialization name..." size="large" class="rounded-xl" />
              </n-form-item>
            </div>
          </n-spin>
        </n-tab-pane>
          ...
          <n-spin :show="isLoadingLang && activeTab === 'ru'">
            <div class="flex flex-col gap-4 pt-4">
              <n-form-item label="Название (RU)">
                <n-input v-model:value="modalForm.title.ru" placeholder="Название специализации..." size="large" class="rounded-xl" />
              </n-form-item>
            </div>
          </n-spin>
        </n-tab-pane>
...
import { useSpecializations, useCreateSpecialization, useUpdateSpecialization, useDeleteSpecialization } from '../composables/useSpecializations'
...
const { specializations, isLoading, error, refresh } = useSpecializations(query)
...
// ---- Composables ----
const { createSpecialization, loading: createLoading } = useCreateSpecialization()
const { updateSpecialization, loading: updateLoading } = useUpdateSpecialization()
const { deleteSpecialization, loading: deleteLoading } = useDeleteSpecialization()
...
const fetchLangData = async (id: number, lang: string) => {
  ...
  try {
    ...
    const data = await $fetch<{ data: Specialization }>(`${BASE_URL}/${id}/`, {
      headers: {
...
const handleSubmit = async () => {
  ...
  try {
    if (editingSpecialization.value) {
      await updateSpecialization(editingSpecialization.value.id, { title: titleObj })
  ...
  },
  {
    ...
    render: (row) =>
      h('div', { class: 'flex items-center gap-3' }, [
        h(NAvatar, { round: true, size: 32, color: '#FFF1F2', style: 'color:#E11D48;font-weight:800;border:1px solid #FFE4E6' },
          { default: () => row.title[0] }),
